...
def compute_cost(AL, Y):
    ...
    error = Y*np.log(AL) + (1 - Y)*np.log(1-AL) #vector with error for each example on a separate row
    cost = -1/m*np.sum(error) #just sum this errors
    ### END CODE HERE ###
...
def linear_activation_backward(dA, cache, activation):
    ...
    if activation == "relu":
        ### START CODE HERE ### (≈ 2 lines of code)
        dZ = relu_backward(dA,activation_cache) #should be dA*g'(Z)
        dA_prev, dW, db = linear_backward(dZ,linear_cache)
        ### END CODE HERE ###
        
    elif activation == "sigmoid":
        ### START CODE HERE ### (≈ 2 lines of code)
        dZ = sigmoid_backward(dA,activation_cache) #should be dA*g'(Z)
        dA_prev, dW, db = linear_backward(dZ,linear_cache)
        ### END CODE HERE ###
...
def L_model_backward(AL, Y, caches):
    ...
    ### START CODE HERE ### (1 line of code)
    dAL = - (np.divide(Y, AL) - np.divide(1 - Y, 1 - AL)) # derivative of cost with respect to AL
    #aka g'(y*logx+(1-y)*log(1-x))
    ### END CODE HERE ###
    
    # Lth layer (SIGMOID -> LINEAR) gradients. Inputs: "dAL, current_cache". Outputs: "grads["dAL-1"], grads["dWL"], grads["dbL"]
    ### START CODE HERE ### (approx. 2 lines)
    current_cache = caches[L]
    #we have the activation of the output layer + weights and biases and activation of the layer before in the current_cache var
    #we use the activation cache aka Z to compute the g'(Z) needed for backprop
    grads["dA" + str(L-1)], grads["dW" + str(L)], grads["db" + str(L)] = linear_activation_backward(dAL,current_cache,"sigmoid")
    #the grads for the output layer, using sigmoid
    ### END CODE HERE ###
    ...
    for l in reversed(range(L-1)):
        ...
        #we have the same stuff in here like above, aka Z[l] value, A[l-1],W[l],b[l]
    ...
    return grads
Jupyter Notebook
Building your Deep Neural Network - Step by Step v8 Last Checkpoint: 3 minutes ago (autosaved)
